...
namespace Rhino.Render
{
    public class SimulatedTexture : IDisposable
    {
        ...
        public enum projection_mode : int
        {
            mapping_channel = 0,
		    view		    = 1,
		    wcs				= 2,
        ...
        public projection_mode ProjectionMode
        {
            get
            {
                return (projection_mode)UnsafeNativeMethods.Rdk_SimulatedTexture_ProjectionMode(m_pSim);
            }
            set
            {
	...
	//void SetColorAdjuster(CRhRdkTexture::IColorAdjuster*);
        ...
        public double UnitsToMeters(double dUnits)
        {
            return UnsafeNativeMethods.Rdk_SimulatedTexture_UnitsToMeters(m_pSim, dUnits);
        }

        public double MetersToUnits(double dUnits)
        {
            return UnsafeNativeMethods.Rdk_SimulatedTexture_MetersToUnits(m_pSim, dUnits);
        }
